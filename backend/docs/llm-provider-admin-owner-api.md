...
当前代码没有独立的 `GET /api/v1/owner/dashboard` 聚合接口。Owner 数据看板应由以下两个已实现接口组合：

| 能力 | 方法 | 路径 | 当前状态 |
| --- | --- | --- | --- |
| 我的任务分页列表 | GET | `/api/v1/owner/tasks` | 已实现 |
| 单任务统计 | GET | `/api/v1/tasks/{taskId}/statistics` | 已实现 |
...
- Controller 路径：`OwnerTaskController`
- 返回类型：`PageResponse<TaskSummaryResponse>`
- 当前实现没有显式 `requireRole(OWNER)`，但会使用当前用户 ID 查询任务；Admin 用户通过 `CurrentUserContext.isAdmin()` 可查询全量任务。

查询参数：

| 参数 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| status | String | 否 | - | 任务状态筛选：`DRAFT`、`PUBLISHED`、`PAUSED`、`ENDED` |
| keyword | String | 否 | - | 按标题或描述关键词搜索 |
| page | Integer | 否 | `1` | 页码，从 1 开始；小于 1 时按 1 处理 |
| size | Integer | 否 | `20` | 每页条数；最小 1，最大 100 |
...
| overlapCount | Integer | 每条数据需要的标注份数 |
| strategy | String | 领取策略 |
...
- Controller 路径：`TaskController`
- 返回类型：`TaskStatisticsResponse`
- 当前实现没有显式 `requireRole(OWNER)`，但普通用户只能查询自己名下任务；Admin 用户可查询任意任务。
...
| --- | --- | --- |
| taskId | Long | 任务 ID |
| totalItems | Integer | 数据集题目总数 |
| claimedCount | Integer | 已领取数 |
| submittedCount | Integer | 已提交、已通过、已打回数量汇总 |
| approvedCount | Integer | 已通过数量 |
| rejectedCount | Integer | 已打回数量 |
| pendingReviewCount | Integer | 待审核数量 |
| passRate | String | 通过率，格式如 `82.00%` |
...
| code | message | 场景 |
| --- | --- | --- |
| 401001 | Unauthorized | 未登录或 token 无效 |
| 403001 | Forbidden | 当前角色不允许调用该接口 |
| 403601 | Reviewer is not assigned to this submission | Reviewer 查询未分配给自己的提交详情 |
| 400102 | range 只支持 7d 或 30d | Admin 看板 `range` 参数非法 |
| 400102 | 请求 JSON 格式或字段类型不合法 | 请求体不是合法 JSON，或字段类型与 DTO 不匹配 |
| 400104 | 创建任务必须配置 AI 审核 | 创建任务时未提供内联 AI 配置必填字段 |
| 400301 | LLM Provider 请求头名称不合法 | Header name 为空或包含换行符 |
| 400401 | 需要配置已启用的 LLM Provider | AI 审核配置引用的 Provider 不存在或未启用 |
| 400402 | AI 审核配置不合法 | 阈值、视觉精度、图片数或评分维度不合法 |
| 400101 | Only draft tasks can be deleted | 删除非草稿任务等任务状态不允许场景 |
| 404001 | 任务不存在 | 查询非自己名下任务或任务 ID 不存在 |
| 404301 | LLM Provider 不存在 | `providerId` 不存在 |
| 404401 | AI 审核配置不存在 | 查询或更新不存在的 AI 审核配置 |
| 404601 | 提交记录不存在 | Reviewer 查询不存在的提交详情 |
| 500301 | LLM API Key 加密密钥未配置 | 未配置 `labelhub.llm.key-encryption-secret` |
| 500302 | LLM API Key 加密或解密失败 | 加密、解密或密文解析失败 |
| 500303 | LLM Provider 请求头配置不合法 | 已保存 Header JSON 无法解析 |

---

## 12. 本轮未提交代码涉及的前端重新对接点
