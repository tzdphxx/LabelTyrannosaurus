...
@Schema(description = "更新 LLM Provider 请求")
public record UpdateLlmProviderRequest(
        ...
        @Schema(description = "Provider 展示名称", example = "DashScope Qwen Max")
        ...
        @Schema(description = "新的 API Key；null、缺省或空白表示保留原有密钥，非空时加密后替换", example = "sk-***")
        @Size(max = 4096) String apiKey,
        @Schema(description = "默认模型名；Owner 缺省 modelName 时使用该模型", example = "qwen-max")
        @NotBlank @Size(max = 128) String defaultModel,
        @Schema(description = "额外请求头；空 key/value 会被忽略，敏感 Header 响应时会被屏蔽")
        Map<String, String> customHeaders,
        @Schema(description = "平台级每分钟限流，0 或 null 表示不限制", example = "120")
        @Min(0) Integer platformRateLimitPerMinute,
        @Schema(description = "任务级每分钟限流，0 或 null 表示不限制", example = "60")
        @Min(0) Integer taskRateLimitPerMinute,
        @Schema(description = "用户级每分钟限流，0 或 null 表示不限制", example = "30")
