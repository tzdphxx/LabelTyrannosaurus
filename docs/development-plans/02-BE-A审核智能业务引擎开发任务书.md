...
> 当前状态说明（2026-06-08）：本文是 BE-A 早期任务书，保留用于 AI Coding 过程记录。最终提交口径以 `docs/final-delivery/` 和当前代码为准：主 Demo 是单人标注链路；冲突/金标能力属于后端保留扩展与历史设计，不作为前端必选流程；LLM Provider 为 ADMIN 全局管理、Owner 查询启用项；AI Flow Policy 支持可配置直通/直拒。
