...
1. **Agent 预标注**：Labeler 在提交前主动调用 AI Agent，生成标注建议。
...
| 位置 | 现有能力 | 局限 |
|---|---|---|
| 标注过程中 | `LlmTrigger` 辅助 | 提供标注建议 |
| 提交之后 | AI 自动预审 | 只辅助 Reviewer，不帮助 Labeler 提高标注效率 |

Agent 预标注用于在提交前提供 AI 辅助建议。
...
> 当前状态说明（2026-06-08）：本文属于增强方向设计记录。当前最终提交材料以 `docs/final-delivery/` 为准：Agent 预标注和多模态链路可作为后端亮点候选；“冲突仲裁已经基本成型”不是当前前端主 Demo 口径，当前代码主流程为单人标注、AI 审核、人工审核、可导出。
