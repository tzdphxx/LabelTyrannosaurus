...
- LLM Provider 会加密保存 API Key；联调和部署环境通过 `LABELHUB_LLM_KEY_ENCRYPTION_SECRET` 配置密钥加密参数。`local` profile 在未配置该变量时提供本地 fallback，便于开发环境跑通接口。
...
**作用**：创建异步导出任务。导出范围为 submission.status=APPROVED 的审核通过提交数据。支持配置导出格式、附加字段和字段映射。
...
24. POST /api/v1/tasks/1/exports
    ...
    说明: 创建异步导出任务（导出 APPROVED 的审核通过数据）
...
- Actuator 指标接口提供 AI 请求量、延迟、状态和错误码维度的运行数据。
