...
|&emsp;&emsp;&emsp;&emsp;max_claims_per_labeler|单人并发未完成上限（仅 QUOTA_GRAB 有效）|integer||
...
|assignmentClaimRequest|AssignmentClaimRequest|body|true|AssignmentClaimRequest|AssignmentClaimRequest|
...
| 状态码 | 说明 | schema |
| -------- | -------- | ----- | 
|200|OK|ApiResponseListAssignmentClaimResponse|
...
|code||integer(int32)|integer(int32)|
|message||string||
|data||array|AssignmentClaimResponse|
|&emsp;&emsp;assignmentId||integer(int64)||
|&emsp;&emsp;datasetItemId||integer(int64)||
|&emsp;&emsp;templateVersionId||integer(int64)||
|&emsp;&emsp;schemaJson||string||
...
|&emsp;&emsp;&emsp;&emsp;returnedReason|上一轮打回原因|string||
|&emsp;&emsp;&emsp;&emsp;returnedAt|上一轮打回时间|string||
|&emsp;&emsp;&emsp;&emsp;updatedAt|更新时间|string||
|traceId||string||
...
|&emsp;&emsp;defaultModel|默认模型名；Owner 缺省 modelName 时使用该模型||true|string||
|&emsp;&emsp;customHeaders|额外请求头；空 key/value 会被忽略，敏感 Header 响应时会被屏蔽||false|object||
|&emsp;&emsp;platformRateLimitPerMinute|平台级每分钟限流，0 或 null 表示不限制||false|integer(int32)||
|&emsp;&emsp;taskRateLimitPerMinute|任务级每分钟限流，0 或 null 表示不限制||false|integer(int32)||
|&emsp;&emsp;userRateLimitPerMinute|用户级每分钟限流，0 或 null 表示不限制||false|integer(int32)||
|&emsp;&emsp;supportVision|是否支持视觉输入；未传按 false 处理||false|boolean||
|&emsp;&emsp;supportMultiImage|是否支持单次请求多图输入；未传按 false 处理||false|boolean||
|&emsp;&emsp;maxImageCount|单次请求最大图片数；未传按 10 处理||false|integer(int32)||
|&emsp;&emsp;visionModel|视觉模型名；为空时视觉请求使用 defaultModel||false|string||
|&emsp;&emsp;structuredOutputMode|结构化输出模式：NONE、JSON_OBJECT、JSON_SCHEMA；未传或非法值保存为空，运行时按不强制结构化输出处理||false|string||
...
|&emsp;&emsp;id|Provider ID，任务 AI 配置中使用该值作为 providerId|integer(int64)||
|&emsp;&emsp;providerCode|Provider 编码，用于后台识别供应商或渠道|string||
|&emsp;&emsp;providerName|Provider 展示名称，前端模型下拉可展示该值|string||
|&emsp;&emsp;baseUrl|OpenAI-compatible Base URL|string||
|&emsp;&emsp;defaultModel|默认模型名，也是 Owner 未显式传 modelName 时实际使用的模型|string||
|&emsp;&emsp;customHeaders|已保存的额外请求头；Authorization、Api-Key 等敏感值会显示为 ******|object||
|&emsp;&emsp;enabled|是否启用；只有启用的 Provider 会出现在 Owner 可选列表|boolean||
|&emsp;&emsp;platformRateLimitPerMinute|平台级每分钟限流；null 表示不使用该层限流|integer(int32)||
|&emsp;&emsp;taskRateLimitPerMinute|任务级每分钟限流；null 表示不使用该层限流|integer(int32)||
|&emsp;&emsp;userRateLimitPerMinute|用户级每分钟限流；null 表示不使用该层限流|integer(int32)||
|&emsp;&emsp;supportVision|是否支持视觉输入|boolean||
|&emsp;&emsp;supportMultiImage|是否支持单次请求多图输入|boolean||
|&emsp;&emsp;maxImageCount|单次请求最大图片数|integer(int32)||
|&emsp;&emsp;visionModel|视觉模型名；为空时视觉请求仍使用 defaultModel|string||
|&emsp;&emsp;structuredOutputMode|结构化输出模式：NONE、JSON_OBJECT、JSON_SCHEMA|string||
|&emsp;&emsp;apiKeyConfigured|是否已配置 API Key；仅表示配置状态，不是密钥内容|boolean||
|&emsp;&emsp;createdBy|创建该 Provider 的管理员用户 ID|integer(int64)||
...
**接口描述**:<p>ADMIN 查看全部 LLM Provider，包括已启用和已停用配置；响应不会返回 API Key 明文或密文。</p>
...
| -------- | -------- | ----- | -------- | -------- | ------ |
|providerId||path|true|integer(int64)||
|testLlmProviderRequest|测试 LLM Provider 连通性请求|body|true|TestLlmProviderRequest|TestLlmProviderRequest|
|&emsp;&emsp;apiKey|临时测试 API Key；未传则使用已保存密钥，临时值不会保存||false|string||
|&emsp;&emsp;modelName|临时测试模型名；未传则使用 Provider defaultModel||false|string||
|&emsp;&emsp;customHeaders|临时附加请求头，会与已保存 Header 合并，临时值不会保存||false|object||
...
|&emsp;&emsp;componentId|模板 ID（整型，仅回显不校验；整模板模式下不再用于定位单个组件）||false|integer(int64)||
...
|&emsp;&emsp;agentRunId||integer(int64)||
|&emsp;&emsp;componentId||integer(int64)||
...
|&emsp;&emsp;&emsp;&emsp;agentRunId||integer||
|&emsp;&emsp;&emsp;&emsp;componentId||integer||
...
**接口地址**:`/api/v1/owner/labelers/assignable`
...
**接口描述**:<p>返回具有标注员角色的用户列表，供任务所有者进行任务分配。</p>
...
| 参数名称 | 参数说明 | 请求类型    | 是否必须 | 数据类型 | schema |
| -------- | -------- | ----- | -------- | -------- | ------ |
|keyword|搜索关键词：用户名、邮箱或显示名称|query|false|string||
|enabledOnly|仅包含已启用且允许登录的标注员|query|false|boolean||
|page|页码，从1开始|query|false|integer(int32)||
|size|每页数量，最大100|query|false|integer(int32)||
...
| 状态码 | 说明 | schema |
| -------- | -------- | ----- | 
|200|OK|ApiResponsePageResponseAssignableLabelerResponse|
...
|code||integer(int32)|integer(int32)|
|message||string||
|data||PageResponseAssignableLabelerResponse|PageResponseAssignableLabelerResponse|
|&emsp;&emsp;items|OWNER 可指派标注员选项|array|AssignableLabelerResponse|
|&emsp;&emsp;&emsp;&emsp;labelerId|标注员用户 ID|integer||
|&emsp;&emsp;&emsp;&emsp;username|用户名|string||
|&emsp;&emsp;&emsp;&emsp;email|邮箱|string||
|&emsp;&emsp;&emsp;&emsp;displayName|显示名称|string||
|&emsp;&emsp;&emsp;&emsp;avatarUrl|头像 URL|string||
|&emsp;&emsp;&emsp;&emsp;enabled|账号是否启用|boolean||
|&emsp;&emsp;&emsp;&emsp;loginEnabled|是否允许登录|boolean||
|&emsp;&emsp;&emsp;&emsp;claimedCount|已领取题目总数量|integer||
|&emsp;&emsp;&emsp;&emsp;submittedCount|已提交题目总数量|integer||
|&emsp;&emsp;&emsp;&emsp;pendingReviewCount|待审核题目数量|integer||
|&emsp;&emsp;&emsp;&emsp;approvedCount|已通过题目数量|integer||
|&emsp;&emsp;&emsp;&emsp;rejectedCount|已驳回题目数量|integer||
|&emsp;&emsp;&emsp;&emsp;totalReward|累计获得奖励|number||
|&emsp;&emsp;&emsp;&emsp;todaySubmittedCount|今日提交题目数量|integer||
|&emsp;&emsp;&emsp;&emsp;lastSubmitDate|最近提交日期|string||
|&emsp;&emsp;&emsp;&emsp;statsUpdatedAt|统计数据更新时间|string||
|&emsp;&emsp;&emsp;&emsp;approvalRate|通过率|number||
|&emsp;&emsp;page||integer(int32)||
|&emsp;&emsp;pageSize||integer(int32)||
|&emsp;&emsp;total||integer(int64)||
|traceId||string||
		...
		"items": [
			{
				...
				"lastSubmitDate": "2026-06-06",
				"statsUpdatedAt": "2026-06-06T10:00:00",
				"approvalRate": 0.75
			}
		],
		"page": 0,
...
**接口地址**:`/api/v1/tasks/{taskId}/items/batch-update`
...
**接口地址**:`/api/v1/tasks/{taskId}/items/batch-delete`
...
**接口地址**:`/api/v1/tasks/{taskId}/items/batch-append`
...
**接口地址**:`/api/v1/tasks/{taskId}/items/batch-append-json`
...
**接口地址**:`/api/v1/tasks/{taskId}/items`
...
**接口地址**:`/api/v1/submissions/{submissionId}/item-history`
...
**接口描述**:<p>从指定提交定位题目，按当前角色返回该题目的提交、AI 审核和多轮人工审核历史。Labeler 只能看到自己的提交历史。</p>
...
| 状态码 | 说明 | schema |
| -------- | -------- | ----- | 
|200|OK|ApiResponseSubmissionItemHistoryResponse|
...
|code||integer(int32)|integer(int32)|
|message||string||
|data||SubmissionItemHistoryResponse|SubmissionItemHistoryResponse|
|&emsp;&emsp;taskId||integer(int64)||
|&emsp;&emsp;datasetItemId||integer(int64)||
|&emsp;&emsp;histories||array|HistoryItem|
|&emsp;&emsp;&emsp;&emsp;submissionId||integer||
|&emsp;&emsp;&emsp;&emsp;assignmentId||integer||
|&emsp;&emsp;&emsp;&emsp;versionNo||integer||
|&emsp;&emsp;&emsp;&emsp;status|可用值:SUBMITTED,AI_REVIEWING,PENDING_FINAL,APPROVED,REJECTED,SUPERSEDED|string||
|&emsp;&emsp;&emsp;&emsp;submittedBy||integer||
|&emsp;&emsp;&emsp;&emsp;submittedByName||string||
|&emsp;&emsp;&emsp;&emsp;submittedAt||string||
|&emsp;&emsp;&emsp;&emsp;aiReview||AiReviewHistory|AiReviewHistory|
...
|&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;reviewedAt||string||
|&emsp;&emsp;&emsp;&emsp;reviewRounds||array|ReviewRoundHistory|
|&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;reviewRecordId||integer||
|&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;reviewLevel||integer||
|&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;reviewerId||integer||
|&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;reviewerName||string||
...
## Create direct export file


**接口地址**:`/api/v1/tasks/{taskId}/exports/direct`
...
**接口描述**:<p>Generate and upload a task-level export file for approved submissions.</p>
...
| -------- | -------- | ----- | -------- | -------- | ------ |
|taskId||path|true|integer(int64)||
|directTaskExportRequest|DirectTaskExportRequest|body|true|DirectTaskExportRequest|DirectTaskExportRequest|
|&emsp;&emsp;format|||false|string||
...
| 状态码 | 说明 | schema |
| -------- | -------- | ----- | 
|200|OK|ApiResponseDirectTaskExportResponse|
...
|code||integer(int32)|integer(int32)|
|message||string||
|data||DirectTaskExportResponse|DirectTaskExportResponse|
...
|&emsp;&emsp;downloadUrl||string||
|&emsp;&emsp;exportedCount||integer(int32)||
|traceId||string||
...
|&emsp;&emsp;assignedLabelerId|ASSIGNED 策略默认指派的标注员 ID|integer(int64)||
|&emsp;&emsp;assignedLabelerName|ASSIGNED 策略默认指派的标注员名称，优先显示名称，缺失时使用用户名|string||
|&emsp;&emsp;publishedTemplateVersionId|已发布模板版本 ID|integer(int64)||
|&emsp;&emsp;aiReview|AI 审核配置响应|AiReviewConfigResponse|AiReviewConfigResponse|
|&emsp;&emsp;&emsp;&emsp;id|配置 ID|integer||
|&emsp;&emsp;&emsp;&emsp;taskId|任务 ID|integer||
|&emsp;&emsp;&emsp;&emsp;providerId|LLM 供应商 ID|integer||
|&emsp;&emsp;&emsp;&emsp;modelName|模型名称|string||
|&emsp;&emsp;&emsp;&emsp;promptTemplate|审核 Prompt 模板|string||
|&emsp;&emsp;&emsp;&emsp;scoringDimensions|评分维度列表|array|string|
|&emsp;&emsp;&emsp;&emsp;passThreshold|通过阈值|number||
|&emsp;&emsp;&emsp;&emsp;manualReviewThreshold|人工复核阈值|number||
|&emsp;&emsp;&emsp;&emsp;outputSchema|输出 JSON Schema|object||
|&emsp;&emsp;&emsp;&emsp;promptVersion|Prompt 版本号|string||
|&emsp;&emsp;&emsp;&emsp;maxRetry|最大重试次数|integer||
|&emsp;&emsp;&emsp;&emsp;aiFlowPolicy|AI 流转策略|string||
|&emsp;&emsp;&emsp;&emsp;allowAiDirectApprove|是否允许 AI 直接通过|boolean||
|&emsp;&emsp;&emsp;&emsp;allowAiDirectReject|是否允许 AI 直接打回|boolean||
|&emsp;&emsp;&emsp;&emsp;rejectThreshold|打回阈值|number||
|&emsp;&emsp;&emsp;&emsp;confidenceThreshold|置信度阈值|number||
|&emsp;&emsp;&emsp;&emsp;riskFlagsForceManual|强制转人工的风险标记|array|string|
|&emsp;&emsp;&emsp;&emsp;multimodalEnabled|是否启用多模态|boolean||
|&emsp;&emsp;&emsp;&emsp;degradationPenalty|多模态降级惩罚系数|number||
|&emsp;&emsp;&emsp;&emsp;visionDetail|视觉精度|string||
|&emsp;&emsp;&emsp;&emsp;maxImagesPerRequest|最大图片数|integer||
|&emsp;&emsp;&emsp;&emsp;allowAiDirectApproveWhenDegraded|降级时是否允许 AI 直接通过|boolean||
|&emsp;&emsp;&emsp;&emsp;reviewStrategy|审核策略: LIGHTWEIGHT | PARALLEL_VOTE | DEEP_DIMENSION | AGENT_DEBATE|string||
|&emsp;&emsp;&emsp;&emsp;voteModels|投票模型列表|array|object|
|&emsp;&emsp;&emsp;&emsp;voteMinAgreement|最少一致票数|integer||
|&emsp;&emsp;&emsp;&emsp;dimensionReviewers|深度模式维度→模型映射|object||
...
```javascript
{
	...
	"data": {
		...
		"assignedLabelerId": 20,
		"assignedLabelerName": "Labeler A",
...
**接口描述**:<p>仅允许编辑 DRAFT 状态任务。
如果传入 aiFlowPolicy 且任务已有 AI 审核配置，后端会同步更新该配置的 AI 流转策略；
如果任务尚无 AI 配置，aiFlowPolicy 不会创建新配置。</p>
...
aiFlowPolicy 可缺省，默认 MANUAL_FIRST。</p>
...
|&emsp;&emsp;aiFlowPolicy|AI 流转策略（内联创建 AI 配置时可选，仅在内联创建时生效）:\nMANUAL_FIRST — AI 只提建议，结果一律转人工（默认）\nAI_PASS_ONLY — 允许 AI 直接过审，打回仍转人工\nAI_REJECT_ONLY — 允许 AI 直接打回，通过仍转人工\nAI_PASS_AND_REJECT — 允许 AI 直接过审与直接打回\nALWAYS_MANUAL — 始终转人工||false|string||
|&emsp;&emsp;strategy|领取策略,可用值:FCFS,QUOTA_GRAB,ASSIGNED||false|string||
|&emsp;&emsp;maxClaimsPerLabeler|单人并发未完成上限（仅 QUOTA_GRAB 有效）||false|integer(int32)||
|&emsp;&emsp;reviewLevelCount|审核级别数（1=单级审核，2=初审+终审，3=初审+复审+终审）||false|integer(int32)||
...
|&emsp;&emsp;&emsp;&emsp;rewardCurrency|奖励货币类型，默认 POINT（平台积分）||false|string||
|&emsp;&emsp;&emsp;&emsp;rewardVisible|奖励是否对标注员可见||false|boolean||
|&emsp;&emsp;assignedLabelerId|ASSIGNED 策略指定的标注员 ID；发布时自动领取全部题目||false|integer(int64)||
